...
using CruiseAssistant.Modules.Standard;
...
namespace CruiseAssistant
{
    class Program
    {
        ...
        private async Task RunAsync()
        {
            ...
            map = new ServiceCollection().AddSingleton(socketClient).AddSingleton(config).AddSingleton(spreadsheet).BuildServiceProvider();
        ...
        private async Task Client_GuildAvailable(SocketGuild guild)
        {
            if (updateChannel != 0 && guild.GetTextChannel(updateChannel) != null)
            {
                ...
                await guild.GetTextChannel(updateChannel).SendMessageAsync("Reconnected.");
                updateChannel = 0;
            }
        }
